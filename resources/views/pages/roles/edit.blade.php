...
@push('breadcrumbs')
    <x-dashboard.breadcrumbs :breadcrumbs="[
            ...
            'Edit' => route('roles.edit', ['role' => $role->id]),
        ]" />
@endpush
...
@section('content')
    ...
    <div class="bg-white rounded-xl shadow-xs p-8 dark:bg-neutral-800">
        <div class="mb-6">
            <h2 class="text-2xl font-semibold text-gray-800 dark:text-white">{{__('role.edit_role')}}</h2>
        </div>

        <form class="space-y-6" action="{{route('roles.update', ['role' => $role->id])}}" method="POST">
            @method('PUT')
            @csrf
            <!-- Role Name -->
            <div class="mb-8">
